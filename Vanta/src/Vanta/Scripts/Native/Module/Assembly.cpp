...
#define SAVE_FUNCTION(name) \
    CONCAT(name, _Impl) = m_Library.GetFunction<CONCAT(name, _Fn)>(#name);
            ...
            SAVE_FUNCTION(GetClassFieldList);
...
#undef SAVE_FUNCTION
        ...
        std::pair<const ClassField*, usize> ScriptAssembly::GetClassFieldList(const char* className) const {
            VANTA_CORE_ASSERT(GetClassFieldList_Impl, "Calling unknown native script function!");
            usize count;
            const ClassField* data = GetClassFieldList_Impl(className, count);
            return { data, count };
        }
